...
def report(request):
    ...
    now = '2021-08-5 11:00' # 현재시간 임시 설정
    ...
    if chk_data == '실시간방문객':
        ...
        now = datetime.strptime(now,'%Y-%m-%d %H:%M') # datetime으로 변환
        nowMinusHour = now - timedelta(hours=1) # 현재시간 - 1시간
        now = str(now)
        nowMinusHour = str(nowMinusHour)
        hour = df.query("@nowMinusHour >= TIME and TIME <= @now")
